...
[![CI/CD](https://img.shields.io/badge/CI%2FCD-GitHub%20Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)](https://github.com/Amritanshu05/ragx/actions)
...
│  ─────────────     │ ┌─────┐ ┌─────┐ ┌──┐│  │ RISK ANALYSIS            │ │
│  Amritanshu P.         │ │  R  │ │  S  │ │P ││  │ [HIGH] score: 7.2/10     │ │
...
RAGX ingests real-time financial data from SEC filings, market prices, and news feeds — indexes it into a FAISS vector store — and runs a coordinated multi-agent pipeline to answer complex financial questions. Every response is a fully structured, typed report covering risk analysis, market sentiment, and portfolio signals, generated in under 3 seconds.
...
│            LiveFeed · PerfChart  —  all wired to useRAGX hook    │
└──────────────────────────┬──────────────────────────────────────────┘
...
The four metric cards — Vectors Indexed, RAG Precision, Queries Today, Agent Success — are populated by polling `GET /health` every 30 seconds via the `useRAGX` hook. The health endpoint reads directly from the live FAISS index (`vector_store.index.ntotal`) and Prometheus counters.
...
```
QueryPanel → useRAGX.analyze() → api.analyze() → POST /analyze
→ RAGXOrchestrator.run()
...
Simulates the Kafka stream from `ingestion/pipeline.py`. In production, this connects to a WebSocket endpoint that consumes from `ragx.news`, `ragx.prices`, and `ragx.filings` Kafka topics and pushes events to the browser in real time. New items animate in at the top of the feed list.
...
1. INGEST (continuous, every 5 minutes)
   ...
   ├── NewsIngester     → NewsAPI → Kafka topic: ragx.news
   ├── MarketPriceIngester → yfinance → Kafka topic: ragx.prices
   └── SECFilingsIngester → SEC EDGAR → Kafka topic: ragx.filings
...
4. RESPOND
   FastAPI → JSON → Axios → useRAGX hook → React components
```
...
│   │   ├── hooks/
│   │   │   └── useRAGX.js    # All state + API calls + agent animation
│   │   ├── services/
...
│       └── service.yaml          # LoadBalancer services + ragx namespace
│
├── monitoring/
...
```bash
git clone https://github.com/Amritanshu05/RAGX.git
cd ragx
cp .env.example .env
...
POSTGRES_URL=postgresql://postgres:password@localhost:5432/ragx
...
|---|---|---|---|
| `GET` | `/health` | Health check, vector count, env | `useRAGX` hook every 30s |
...
cd backend
docker build -t ragx .
docker tag ragx:latest <ECR_URL>/ragx:latest
docker push <ECR_URL>/ragx:latest
```
...
kubectl rollout status deployment/ragx-api -n ragx
kubectl rollout status deployment/ragx-frontend -n ragx
```
