...
DEFAULT_ARGS = {
    ...
    "email": ["amritanshu0507@gmail.com"],
}
...
def retrain_embeddings(**ctx):
    ...
    mlflow.set_experiment("ragx-embedding-finetune")
    with mlflow.start_run(run_name="embedding-finetune"):
        ...
        model.save("/tmp/ragx-embeddings-finetuned")

        s3 = boto3.client("s3", region_name="eu-west-2")
        s3.upload_file("/tmp/ragx-embeddings-finetuned/config.json",
                       os.getenv("S3_BUCKET", "ragx-prod"), "models/embeddings/config.json")
...
def notify_success(**ctx):
    logger.info("RAGX retraining pipeline complete")


with DAG(
    dag_id="ragx_retraining_pipeline",
    default_args=DEFAULT_ARGS,
    description="Daily ingestion + drift detection + auto-retraining",
    schedule_interval="0 2 * * *",
    catchup=False,
    tags=["ragx", "ml", "rag", "retraining"],
) as dag:
